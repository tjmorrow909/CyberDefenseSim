        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {scenarios.map(scenario => (
            <Link key={scenario.id} href={`/scenario/${scenario.id}`}>
              <div
                className={`group cursor-pointer border border-border rounded-lg p-6 hover:shadow-md transition-all duration-200 ${getHoverBorderColor(scenario.type)}`}
              >
                <div className="flex items-center justify-between mb-4">
                  <div
                    className={`w-12 h-12 rounded-lg flex items-center justify-center ${getIconBgColor(scenario.type)}`}
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <ArrowRight
                      className={`w-4 h-4 text-muted-foreground transition-colors ${getArrowHoverColor(scenario.type)}`}
                    />
                  </div>
                </div>
              </div>
